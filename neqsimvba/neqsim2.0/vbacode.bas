...
Sub InitializeThermo()
    ...
    Call fluidSystem.createFluid
    Call fluidSystem.addComponent(Range("A4"), Range("B4").Value)
    Call fluidSystem.addComponent(Range("A5"), Range("B5").Value)
    Call fluidSystem.addComponent(Range("A6"), Range("B6").Value)
    Call fluidSystem.addComponent(Range("A7"), Range("B7").Value)
    Call fluidSystem.addComponent(Range("A8"), Range("B8").Value)
    Call fluidSystem.setMixingRule(2)
    Call fluidSystem.initFluid(0)
...
Sub TPflash(T As Double, P As Double, Optional ByVal flashType As Integer = -1, Optional ByVal initType As Integer = 3)
    ...
    Select Case flashType
        ...
        Case 1
            ...
            Call fluidSystem.init(phaseType, initType)
        Case Is <= -1
            Call fluidSystem.TPflash
            Call fluidSystem.initFluid(initType)
    End Select
...
Function density(T As Double, P As Double, x As Range, Optional ByVal flashType As Integer = -1, Optional ByVal phaseNumber As Integer = -1) As Double
    Call setTPFraction(T, P, x)
    Call TPflash(T, P, flashType, 1)
    Call fluidSystem.getThermoSystem().initPhysicalProperties
    If flashType < 0 Then
        If phaseNumber = -1 Then
            density = fluidSystem.getThermoSystem().getDensity("kg/m3")
        Else
            density = fluidSystem.getThermoSystem().getPhase(phaseNumber).getDensity("kg/m3")
        End If
    Else
        density = fluidSystem.getThermoSystem().getDensity("kg/m3")
    End If
...
Function viscosity(T As Double, P As Double, x As Range, Optional ByVal flashType As Integer = -1, Optional ByVal phaseNumber As Integer = -1) As Double
    ...
    Call fluidSystem.getThermoSystem().initPhysicalProperties
     ...
     If phaseNumber = -1 Then
        ...
        Else
            viscosity = fluidSystem.getThermoSystem().getPhase(phaseNumber).getViscosity()
        End If
    Else
        viscosity = fluidSystem.getThermoSystem().getViscosity()
    End If
...
Function conductivity(T As Double, P As Double, x As Range, Optional ByVal flashType As Integer = -1, Optional ByVal phaseNumber As Integer = -1) As Double
    ...
    Call fluidSystem.getThermoSystem().initPhysicalProperties
     If flashType < 0 Then
        If phaseNumber = -1 Then
            conductivity = fluidSystem.getThermoSystem().getThermalConductivity()
        Else
            conductivity = fluidSystem.getThermoSystem().getPhase(phaseNumber).getThermalConductivity()
        End If
    Else
        conductivity = fluidSystem.getThermoSystem().getThermalConductivity()
    End If
